...
//    private EntityManager entityManager;
    ...
    public HealthCheckDTO getStatusSystem(boolean isDetail) {
        ...
        try {
            ...
            healthCheckDTO.setRequestTime(DateTimeUtils.convertToString(new Date(), DateTimeUtils.YYYY_MM_DD_T_HH_MM_SSZ));
            if (!isDetail) {
                return healthCheckDTO;
            }
            healthCheckDTO.setActiveProfile(appConfiguration.getProfileActive());
...
//            healthCheckDTO.setRequestTime(DateTimeUtils.convertToString(new Date(), DateTimeUtils.YYYY_MM_DD_T_HH_MM_SSZ));
//            Query query = entityManager.createQuery(QueryConst.SELECT_1);
