...
public enum ErrorCodeEnum {
    ...
    /**
     * set error from old response
     *
     * @param oldResponse old response
     * @param newResponse new response
     * @param <T>,<E>     must extends ResponseDTO
     */
    public static <T extends ResponseDTO, E extends ResponseDTO> void setNewResponse(T oldResponse, E newResponse) {
        newResponse.setErrorCode(oldResponse.getErrorCode());
        newResponse.setMessage(oldResponse.getMessage());
        if (ValidateUtils.isNotNullOrEmpty(oldResponse.getHttpStatus())) {
            newResponse.setHttpStatus(oldResponse.getHttpStatus());
        }
    }
